...
- Trust your ear over the algorithm when they disagree — the tool is a study aid; you are the ground truth.
...
- Re-analyze with different settings via **Tools → Reanalyze** when something looks off; a different stem-quality or chord vocabulary sometimes catches what the default missed.
...
**Manual / human path:** [`INSTALL.md`](INSTALL.md) is the same content as a human-readable runbook. It assumes you can follow PowerShell + WSL commands. It's the long way around but it works.
...
- **~970 Python tests pass** (analyze + webui). Rough counts: ~550 in `tests/`, ~420 in `webui/tests/`. Run `pytest --collect-only -q` for exact numbers.
- Plus Playwright e2e specs and Node pure-logic specs in `webui/tests-e2e/` and `webui/tests-js/`.
- **Batch-validated April 2026** on 5 mixed-genre MP3s — two real bugs found and fixed during validation (librosa duration on malformed Xing/VBR headers; `vocal_range` leaking from instrumental content). See [`install-logs/batch-test-results.md`](install-logs/batch-test-results.md).
- **Post-validation work** (2026-05): vocal consensus pipeline (Phase 0c), theme tokens + 5 presets, identify-pipeline overhaul (5 rounds), WASAPI audio engine v1, live mic-pitch layer, sidebar polish. See [`docs/history.md`](docs/history.md) for chronology.
...
I make and study music, and I wanted a single surface that could keep up with the questions I have about a song while I'm listening to it: what chord is that, what's the singer doing right now, where is the kick relative to the snare, can I sing this line in tune. None of the off-the-shelf options sat in that exact spot, so this. If it sounds useful for your own practice or analysis, hand the repo to an agent and ask it to set it up — that's the cleanest path. If you want to fork it and take it somewhere of your own, the code is open and the architecture is documented.

— Radu (`<maintainer-email>`)
