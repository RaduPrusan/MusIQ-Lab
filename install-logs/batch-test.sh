#!/usr/bin/env bash
# Batch test analyze.py on the user's local MP3 corpus.
#
# Override the source directory with MUSIQ_MP3_DIR (defaults to
# ~/Music/musiq-lab-test). Override the project root with MUSIQ_PROJECT_DIR
# (defaults to the repo this script is running from).

set -uo pipefail  # NOT -e: a single-track failure shouldn't abort the rest.

cd "${MUSIQ_PROJECT_DIR:-$(git -C "$(dirname "$0")" rev-parse --show-toplevel)}"
source .venv/bin/activate

MP3_DIR="${MUSIQ_MP3_DIR:-$HOME/Music/musiq-lab-test}"
TRACKS=(
  "Autumn Leaves - Chet Baker & Paul Desmond Together [sgn7VfXH2GY].mp3"
  "Charlie Puth - Attention.mp3"
  "Lou Reed - Perfect Day (Official Audio)-9wxI4KK9ZYo.mp3"
  "Orchestral Suite No. 3 in D Major_ II. Air on a G-String (Arr. for Cello Quintet)-Ing6BtC4S0A.mp3"
  "The Autumn leaves - Gm (130bpm) - Backing Track.mp3"
)

echo "==== batch-test starting at $(date -u +%Y-%m-%dT%H:%M:%SZ) ===="
echo "tracks: ${#TRACKS[@]}"

for i in "${!TRACKS[@]}"; do
  mp3="$MP3_DIR/${TRACKS[$i]}"
  short="${TRACKS[$i]%.mp3}"
  echo
  echo "==== [$((i+1))/${#TRACKS[@]}] $(date -u +%H:%M:%SZ): ${short:0:80} ===="
  if [ ! -r "$mp3" ]; then
    echo "!!  FILE NOT READABLE: $mp3"
    continue
  fi

  start=$(date +%s)
  python -m analyze "$mp3" 2>&1 | sed 's/^/    /'
  rc=${PIPESTATUS[0]}
  elapsed=$(( $(date +%s) - start ))
  echo "==== exit=$rc, ${elapsed}s ===="

  # If success, dump a quick summary
  if [ "$rc" -eq 0 ]; then
    # Find the cache slug for this MP3 by re-deriving via python
    slug=$(python -c "from pathlib import Path; from analyze.cache import slug_for; print(slug_for(Path('$mp3')))")
    summary="cache/$slug/$slug.summary.json"
    if [ -r "$summary" ]; then
      python -c "
import json
s = json.load(open('$summary'))
t = s['track']
a = s['analysis']
print(f\"    key={t['key']} (conf={t['key_confidence']:.2f})\")
print(f\"    tempo={t['tempo_bpm']:.1f} BPM\")
print(f\"    duration={t['duration_sec']:.1f}s\")
print(f\"    scale={a['scale']}\")
print(f\"    chords={len(s['chords'])}\")
print(f\"    loop={a['predominant_chord_loop']}\")
print(f\"    loop_roman={a['loop_roman']}\")
print(f\"    modal_interchange_count={a['modal_interchange_count']}\")
print(f\"    vocal_range={a['vocal_range']}\")
for stem, info in s['stems'].items():
    if 'notes' in info: print(f\"    notes[{stem}]={len(info['notes'])}\")
print(f\"    warnings={s['provenance']['warnings']}\")
"
    fi
  fi
done

echo
echo "==== batch-test complete at $(date -u +%Y-%m-%dT%H:%M:%SZ) ===="
